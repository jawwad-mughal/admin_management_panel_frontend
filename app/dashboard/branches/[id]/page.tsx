...
export default function Dashboard() {
  ...
  if (!data) return <LoadingCard message="Loading branch data..." className="h-[70vh]" />;
    ...
    <div className="space-y-6 ">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1 text-sm">Branch performance and order overview</p>
        </div>
        <button onClick={() => router.back()} className="flex items-center gap-2 px-3 py-1.5 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm transition-colors duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <StatCard title="Total Orders" value={data.totalOrders} icon={<FiShoppingCart className="w-5 h-5" />} color="blue" />
        <StatCard title="Revenue" value={`Rs ${data.totalRevenue}`} icon={<FiDollarSign className="w-5 h-5" />} color="green" />
        <StatCard title="Pending" value={data.pending || 0} icon={<FiClock className="w-5 h-5" />} color="yellow" />
        <StatCard title="Delivered" value={data.delivered || 0} icon={<FiCheckCircle className="w-5 h-5" />} color="emerald" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <h2 className="text-md font-semibold text-gray-900 dark:text-white mb-3">Revenue Over Time</h2>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <h2 className="text-md font-semibold text-gray-900 dark:text-white mb-2">Order Status</h2>
      ...
      <div className="space-y-3">
        ...
        {data.orders.length ? (
          data.orders.map((o: any) => {
            ...
            return (
              <div key={o._id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  <span className="font-mono text-sm text-gray-900 dark:text-white">#{o._id.slice(-8)}</span>
...
// StatCard Component
function StatCard({ title, value, icon, color = "blue" }: { title: string; value: any; icon: any; color?: string }) {
  const colorClasses = {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between hover:shadow-lg transition-all duration-200">
      ...
      <div className={`w-10 h-10 ${colorClasses[color as keyof typeof colorClasses]} rounded-xl flex items-center justify-center shadow-lg text-white`}>
        {icon}
      </div>
    </div>
  );
